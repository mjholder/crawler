...
static func compute(action: Object, source: Node) -> AttackPreview:
	...
	for effect in effects:
		preview._append_effect(effect, eval, source)
	return preview


func _append_effect(effect: Resource, eval: StatExprEval, source: Node) -> void:
	if effect == null:
		return
	...
	if effect is StatusEffect:
		...
		heal.value = eval.evaluate(effect.heal_expression, source)
		heal.label = humanize_expression(effect.heal_expression)
		lines.append(heal)
	elif effect is BuffEffect:
		var buff := Line.new()
		buff.kind = LineKind.BUFF
		buff.value = eval.evaluate(effect.amount_expression, source)
		...
		brace.value = eval.evaluate(effect.amount_expression, source)
		brace.label = "armor"
		lines.append(brace)
...
## dumping source (item 5 of the legibility idea).
static func humanize_expression(expression: String) -> String:
	var out := expression
